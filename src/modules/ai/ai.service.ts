...
export class AIService {
  private readonly gen = new GoogleGenerativeAI(process.env.GEMINI_API_KEY as string)
  private readonly modelText = this.gen.getGenerativeModel({ model: 'gemini-1.5-flash' })

  async embed(text: string): Promise<number[]> {
    const embModel = this.gen.getGenerativeModel({ model: 'text-embedding-004' })
    const r = await (embModel as any).embedContent(text)
    return (r.embedding.values as number[]) || []
  }

  async normalizeEvent(input: { title: string; description?: string; venue?: string; tags?: string[] }): Promise<{ categories: string[]; tags: string[]; mappedInterests: { id: string; weight: number }[]; rationale: string }> {
  ...
  async buildTwoPlans(input: { title: string; venue?: string; address?: string; start?: string }): Promise<Array<{ title: string; description: string; emoji?: string }>> {
