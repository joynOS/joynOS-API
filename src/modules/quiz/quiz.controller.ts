...
export class QuizController {
  ...
  async submit(
    ...
    for (const a of body.answers) {
      ...
      if (ans?.archetype)
        tally[ans.archetype] = (tally[ans.archetype] || 0) + 1;
    }
    const dominantArchetype =
      Object.entries(tally).sort((a, b) => b[1] - a[1])[0]?.[0] || null;
    await this.prisma.userQuizResult.upsert({
      where: { userId_quizId: { userId, quizId } },
      update: {
        archetypeTally: tally as any,
        dominantArchetype: dominantArchetype || undefined,
      },
      create: {
        userId,
        quizId,
    ...
    await this.prisma.user.update({
      where: { id: userId },
      data: {
        aiProfile: { tally, dominantArchetype } as any,
