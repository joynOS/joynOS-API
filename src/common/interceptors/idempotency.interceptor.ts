...
export class IdempotencyInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler<any>) {
    ...
    const hash = createHash('sha256').update(JSON.stringify(res ?? {})).digest('hex')
    await this.prisma.idempotencyKey.create({ data: { key, method: req.method, path: req.originalUrl || req.url, userId: req.user?.userId, statusCode: 200, responseHash: hash } })
    return res
  }
}
